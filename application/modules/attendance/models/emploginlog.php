...
class Default_Model_Userloginlog extends Zend_Db_Table_Abstract
{
	...
	public function getUserLoginLogGrid($sort,$by,$perPage,$pageNo,$searchData,$call,$p1,$p2,$p3,$p4,$p5)
	{
		...
		if($searchData != '' && $searchData!='undefined')
		{
			...
			if(count($searchValues) >0)
			{
				foreach($searchValues as $key => $val)
				{
					...
					if($key == 'userfullname' || $key == 'employeeId' || $key == 'emailaddress' || $key == 'emailaddress' || $key == 'date'|| $key == 'checkintime'|| $key == 'checkinaddress' || $key == 'checkouttime' || $key == 'checkoutaddress' )
					{
		...
		$tableFields = array('action'=>'Action',
                             	...
                             	'rolename' => 'Role',
                             	'emailaddress'=>'Email',
														 	'checkintime' => 'Checkinn Time',
														 	'checkinaddress' =>'Checkedin Address'
														 	'checkouttime' => 'Checkout Time',
														 	'checkoutaddress' => 'Checkedout Address'
		);
